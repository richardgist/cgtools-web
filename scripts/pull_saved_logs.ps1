...
if (-not $LocalDir.Trim()) {
    ...
    $LocalDir = Join-Path $performanceDataRoot 'Logs'
}

if (-not (Test-Path $LocalDir)) {
    New-Item -ItemType Directory -Path $LocalDir | Out-Null
    Write-Host "Created logs directory: $LocalDir"
}

Write-Host '===================================='
Write-Host '  Saved Logs Directory Puller'
Write-Host '===================================='
Write-Host ''
...
$remoteRoots = @(
    "/sdcard/Android/data/$PackageName/files/UE4Game/$ProjectName/$ProjectName/Saved/Logs",
    "/sdcard/UE4Game/$ProjectName/$ProjectName/Saved/Logs"
)
...
if (-not $remoteLogsDir) {
    Write-Host ''
    Write-Host '[ERROR] Saved/Logs directory not found.'
...
$destination = Join-Path $LocalDir ("{0}_Logs_{1}" -f $ProjectName, $timestamp)

Write-Host ''
Write-Host 'Pulling Saved/Logs directory...'
Write-Host "  Remote: $remoteLogsDir"
Write-Host "  Local: $destination"

Invoke-Adb -Arguments @('pull', $remoteLogsDir, $destination) | Out-Null

Write-Host ''
Write-Host '===================================='
Write-Host '  Saved/Logs pull completed!'
Write-Host '===================================='
