<template>
  <div class="page active" style="display: flex;">
    ...
    <!-- 维基体检 (Wiki Lint) Modal -->
    <Teleport to="body">
      <div v-if="wikiLintVisible" class="kc-modal-overlay" @click.self="closeWikiLintModal">
        <div class="kc-modal kc-create-modal" style="max-width: 1000px;">
          <button class="kc-modal-close" @click="closeWikiLintModal">×</button>
          <h2 class="kc-modal-title" style="color: #e056fd; text-shadow: 0 0 10px rgba(224, 86, 253, 0.35);">🧹 维基体检 (Wiki Lint) 自愈控制台</h2>
          
          <div v-if="wikiLintRunning" class="wiki-lint-loading-panel">
            <div class="wiki-lint-radar-container">
              <div class="wiki-lint-radar">
                <div class="wiki-lint-radar-beam"></div>
              </div>
            </div>
            <p class="wiki-lint-progress-text">正在实时分析双向引用图谱，诊断脑网络健康度... {{ wikiLintProgress }}%</p>
            <div class="wiki-lint-progress-bar-wrapper">
              <div class="wiki-lint-progress-bar" :style="{ width: wikiLintProgress + '%' }"></div>
            </div>
          </div>

          <div v-else>
            <div class="wiki-lint-overview">
              <div class="wiki-lint-overview-card">
                <span class="wiki-lint-overview-num">{{ wikiLintStats.totalCards }}</span>
                <span class="wiki-lint-overview-label">卡片总数</span>
              </div>
              <div class="wiki-lint-overview-card warning" :class="{ 'has-issue': wikiLintStats.brokenLinksCount > 0 }">
                <span class="wiki-lint-overview-num">{{ wikiLintStats.brokenLinksCount }}</span>
                <span class="wiki-lint-overview-label">🔗 概念断链</span>
              </div>
              <div class="wiki-lint-overview-card info" :class="{ 'has-issue': wikiLintStats.orphansCount > 0 }">
                <span class="wiki-lint-overview-num">{{ wikiLintStats.orphansCount }}</span>
                <span class="wiki-lint-overview-label">🕸️ 孤立卡片</span>
              </div>
            </div>

            <div class="wiki-lint-split-layout">
              <!-- 左栏：概念断链 -->
              <div class="wiki-lint-section">
                <div class="wiki-lint-section-title">
                  <span>🔗 概念断链 ({{ wikiLintStats.brokenLinksCount }})</span>
                  <span class="wiki-lint-section-desc">正文中引用的 [[KC-xxxx]] 并不存在</span>
                </div>
                <div class="wiki-lint-list">
                  <div v-if="wikiLintBrokenLinks.length === 0" class="wiki-lint-empty">
                    🎉 零断链，链接极其健康！
                  </div>
                  <div v-else v-for="link in wikiLintBrokenLinks" :key="link.sourceId + '-' + link.targetId" class="wiki-lint-item broken">
                    <div class="wiki-lint-item-main">
                      <div class="wiki-lint-item-title">
                        来自卡片: <a class="wiki-link-interactive" @click="openCardById(link.sourceId)">{{ link.sourceTitle }} ({{ link.sourceId }})</a>
                      </div>
                      <div class="wiki-lint-item-detail">
                        指向未知目标: <code class="broken-id">{{ link.targetId }}</code>
                      </div>
                      <!-- 智能自愈建议 -->
                      <div class="wiki-lint-healing-suggestions">
                        <span class="healing-sug-title">💡 推荐自愈方案：</span>
                        <div class="healing-options">
                          <button 
                            v-for="sug in getBrokenLinkSuggestions(link.targetId)" 
                            :key="sug.id"
                            class="fluent-btn fluent-btn-sm" 
                            style="margin: 2px;"
                            @click="healBrokenLink(link.sourceId, link.targetId, sug.id)"
                          >
                            替换为: {{ sug.title }} ({{ sug.id }})
                          </button>
                          <span v-if="getBrokenLinkSuggestions(link.targetId).length === 0" class="no-sug">无相似概念卡片，支持一键清理该引用</span>
                        </div>
                        <!-- 一键自动清理引用按钮 -->
                        <div style="margin-top: 8px; border-top: 1px dashed rgba(255, 255, 255, 0.05); padding-top: 8px;">
                          <button 
                            class="fluent-btn fluent-btn-sm danger" 
                            style="width: 100%; justify-content: center; background: rgba(231, 76, 60, 0.12); border-color: rgba(231, 76, 60, 0.28); color: #ff7675;"
                            @click="removeBrokenLink(link.sourceId, link.targetId)"
                          >
                            🗑️ 一键清理此失效引用
                          </button>
                        </div>
              ...
              <div class="wiki-lint-section">
                <div class="wiki-lint-section-title">
                  <span>🕸️ 孤立卡片 ({{ wikiLintStats.orphansCount }})</span>
                  <span class="wiki-lint-section-desc">未被任何卡片引用，且自身也未引用其它卡片</span>
                </div>
                <div class="wiki-lint-list">
                  <!-- 全局一键全量自愈编织大按钮 -->
                  <div v-if="wikiLintOrphans.length > 0" style="margin-bottom: 12px; padding: 0 4px;">
                    <button 
                      class="fluent-btn primary" 
                      style="width: 100%; justify-content: center; background: linear-gradient(135deg, #e056fd, #9b5de5); box-shadow: 0 0 12px rgba(224, 86, 253, 0.35); font-weight: bold; padding: 8px 12px; font-size: 12px;"
                      :disabled="wikiLintRunning"
                      @click="autoHealAllOrphans"
                    >
                      ⚡ 一键全量自愈编织 (同步全部 {{ wikiLintStats.orphansCount }} 张卡片)
                    </button>
                  </div>
                  <div v-if="wikiLintOrphans.length === 0" class="wiki-lint-empty">
                    🎉 零孤立，知识网络已全面织网！
                  </div>
                  <div v-else v-for="card in wikiLintOrphans" :key="card.id" class="wiki-lint-item orphan">
                    <div class="wiki-lint-item-main">
                      <div class="wiki-lint-item-title">
                        <a class="wiki-link-interactive" @click="openCardById(card.id)">{{ card.title }} ({{ card.id }})</a>
                      </div>
                      <div class="wiki-lint-item-detail">{{ card.summary || '无摘要说明' }}</div>
                      <div class="wiki-lint-healing-actions" style="margin-top: 8px;">
                        <button class="fluent-btn fluent-btn-sm primary" @click="autoHealOrphan(card)">
                          ⚡ 一键索引织网 (同步Obsidian)
                        </button>
                      </div>
                    </div>
          ...
          <div class="kc-create-actions" style="margin-top: 24px;">
            <button class="fluent-btn" @click="closeWikiLintModal">关闭</button>
            <button class="fluent-btn primary" :disabled="wikiLintRunning" @click="runWikiLint" style="background: linear-gradient(135deg, #e056fd, #9b5de5);">
              🚀 重新扫描体检
            </button>
          </div>
        </div>
...
function openWikiLintModal() {
  wikiLintVisible.value = true
  runWikiLint()
}

function closeWikiLintModal() {
  wikiLintVisible.value = false
}

async function runWikiLint() {
  wikiLintRunning.value = true
  wikiLintProgress.value = 10
  
  const timer = setInterval(() => {
    if (wikiLintProgress.value < 90) {
      wikiLintProgress.value += 15
    }
  }, 100)
  ...
  try {
    const data = await $fetch<{
      ok: boolean
      stats: { totalCards: number; brokenLinksCount: number; orphansCount: number }
      brokenLinks: { sourceId: string; sourceTitle: string; targetId: string }[]
      orphans: { id: string; title: string; summary: string }[]
    }>('/api/knowledge/lint')
    
    clearInterval(timer)
    wikiLintProgress.value = 100
    
    setTimeout(() => {
      wikiLintStats.value = data.stats
      wikiLintBrokenLinks.value = data.brokenLinks || []
      wikiLintOrphans.value = data.orphans || []
      wikiLintRunning.value = false
    }, 200)
  } catch (e) {
    clearInterval(timer)
    wikiLintRunning.value = false
    console.error('Failed to run Wiki Lint:', e)
  }
}

function getBrokenLinkSuggestions(targetId: string) {
  if (!targetId) return []
  const cleanTarget = targetId.toLowerCase().replace(/[^a-z0-9]/g, '')
  return cards.value
    .map(card => {
      const cleanCardId = card.id.toLowerCase().replace(/[^a-z0-9]/g, '')
      let similarity = 0
      if (cleanCardId.includes(cleanTarget) || cleanTarget.includes(cleanCardId)) {
        similarity += 50
      }
      const targetSeq = targetId.split('-').pop() || ''
      const cardSeq = card.id.split('-').pop() || ''
      if (targetSeq && cardSeq && targetSeq === cardSeq) {
        similarity += 40
      }
    ...
    .slice(0, 3)
    .map(item => ({ id: item.card.id, title: item.card.title }))
}

async function healBrokenLink(sourceId: string, targetId: string, resolvedId: string) {
  try {
    const data = await $fetch<{ markdown: string }>(`/api/knowledge/${encodeURIComponent(sourceId)}`)
    let markdown = data.markdown || ''
    if (!markdown) return
    
    const escapedTarget = targetId.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, '\\$&')
    const regex = new RegExp(`\\[\\[(${escapedTarget})(\\|[^\\]]+)?\\]\\]`, 'g')
    markdown = markdown.replace(regex, `[[${resolvedId}$2]]`)
    
    await $fetch(`/api/knowledge/${encodeURIComponent(sourceId)}`, {
      method: 'PUT',
      body: { markdown }
    })
    ...
    console.error('Failed to auto-heal broken link:', e)
    alert('修复断链失败: ' + String(e))
  }
}

async function removeBrokenLink(sourceId: string, targetId: string) {
  try {
    const data = await $fetch<{ markdown: string }>(`/api/knowledge/${encodeURIComponent(sourceId)}`)
    let markdown = data.markdown || ''
    if (!markdown) return
    ...
    console.error('Failed to remove broken link:', e)
    alert('清除失效引用失败: ' + String(e))
  }
}

async function autoHealOrphan(card: any) {
  wikiLintRunning.value = true
  wikiLintProgress.value = 10
  try {
    await rebuildIndex()
    wikiLintProgress.value = 60
    await runWikiLint()
  } catch (e) {
    console.error('Failed to heal orphan:', e)
  } finally {
    wikiLintRunning.value = false
  }
}

async function autoHealAllOrphans() {
  wikiLintRunning.value = true
  wikiLintProgress.value = 10
  try {
    await rebuildIndex()
    ...
    console.error('Failed to auto-heal all orphans:', e)
  } finally {
    wikiLintRunning.value = false
  }
}
...
async function openCardById(cardId: string) {
  ...
  if (!card) {
    let title = cardId
    const broken = wikiLintBrokenLinks.value.find(l => l.sourceId === cardId)
    if (broken) {
      title = broken.sourceTitle || cardId
    } else {
      const orphan = wikiLintOrphans.value.find(o => o.id === cardId)
      if (orphan) {
        title = orphan.title || cardId
      }
    }
    
    let domain = '未分类'
    if (cardId.startsWith('HUB-')) domain = 'HUB'
    else if (cardId.startsWith('MOC-')) domain = 'MOC'
    else if (cardId === '00-知识体系总图') domain = 'MAP'
