...
export default defineEventHandler(async (event) => {
  const body = await readBody<DeleteRemotePakBody>(event)
  ...
  let plan
  try {
    plan = createRemotePakDeletePlan({
    ...
    throw createError({
      statusCode: 400,
      statusMessage: error instanceof Error ? error.message : '创建手机 Pak 删除计划失败',
    })
  }
